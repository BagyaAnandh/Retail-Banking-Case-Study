...
SELECT *
FROM address a
JOIN online_customer o_c
ON a.ADDRESS_ID = o_c.ADDRESS_ID
JOIN order_header o_h
ON o_c.CUSTOMER_ID = o_h.CUSTOMER_ID
JOIN order_items o_i
ON o_h.ORDER_ID = o_i.ORDER_ID
JOIN product p
ON o_i.PRODUCT_ID = p.PRODUCT_ID 
JOIN product_class p_c
ON p.PRODUCT_CLASS_CODE = p_c.PRODUCT_CLASS_CODE;
...
SELECT p.PRODUCT_DESC, p_c.PRODUCT_CLASS_DESC, p.PRODUCT_PRICE, o_h.ORDER_STATUS
FROM product p
JOIN order_items o_i
ON p.PRODUCT_ID = o_i.PRODUCT_ID
JOIN order_header o_h
ON o_h.ORDER_ID = o_i.ORDER_ID
CROSS JOIN product_class p_c
ON p_c.PRODUCT_CLASS_CODE = p.PRODUCT_CLASS_CODE
WHERE o_h.ORDER_STATUS LIKE 'Shipped';
...
SELECT PRODUCT_CLASS_CODE, PRODUCT_DESC, PRODUCT_QUANTITY_AVAIL,
CASE 
	WHEN PRODUCT_QUANTITY_AVAIL = 0 THEN 'OUT OF STOCK'
	WHEN PRODUCT_CLASS_CODE = 2050 AND PRODUCT_CLASS_CODE = 2053 THEN
		CASE 
		...
		END
	WHEN PRODUCT_CLASS_CODE = 2052 AND PRODUCT_CLASS_CODE = 2056 THEN
		CASE 
...
SELECT DISTINCT o_c.CUSTOMER_ID, o_c.CUSTOMER_FNAME, o_c.CUSTOMER_LNAME, o_c.CUSTOMER_EMAIL, a.STATE
FROM address a
JOIN online_customer o_c
ON a.ADDRESS_ID = o_c.ADDRESS_ID
JOIN order_header o_h
...
JOIN product_class p_c
ON p.PRODUCT_CLASS_CODE = p_c.PRODUCT_CLASS_CODE
WHERE p_c.PRODUCT_CLASS_CODE != 2051 AND p_c.PRODUCT_CLASS_CODE != 2054
HAVING STATE NOT LIKE 'karnataka';
